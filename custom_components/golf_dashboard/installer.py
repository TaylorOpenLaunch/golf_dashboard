...
async def _get_or_create_dashboard(hass: HomeAssistant):
    ...
    try:
        dashboard = await ll_dashboard.async_get_dashboard(hass, DASHBOARD_URL_PATH)
    ...
    try:
        return await ll_dashboard.async_create_dashboard(
            hass,
            DASHBOARD_URL_PATH,
            {"views": []},
            title=DASHBOARD_TITLE,
        ...
        _LOGGER.exception("Golf Dashboard: failed to create storage dashboard")
